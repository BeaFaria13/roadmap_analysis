    ...
    with stylable_container(
        ...
        with radar_chart:
            ...
            dataframe = pd.DataFrame(data={
                'média obtida':avg_per_transf['Média'],
                'transformação':avg_per_transf['Transformação']})
            
            # create main radar chart
            # fig = go.Figure()
            # dataframe['média obtida'].append(dataframe['média obtida'][0])
            # dataframe['transformação'].append(dataframe['transformação'][0])

            # chart_results=go.Scatterpolar(
            #     r=dataframe['média obtida'],
            #     theta=dataframe['transformação'],
            #     fillcolor='#37a7a8')
            fig=px.line_polar(data_frame=dataframe, r='média obtida', theta='transformação', line_close=True,markers=True)
            fig.update_traces(marker_color='#37a7a8',line_color='#37a7a8')
            fig.update_layout({'font_size':14})


            # highlight_trace = go.Scatterpolar(
            #     r=[4]*len(dataframe['média obtida']),  # creating a "circle"
            #     theta=dataframe['transformação'],
            #     mode='lines',
